...
export class ImageListComponent implements OnInit, OnDestroy {
  ...
  private getCardTable(): void {
    this.subscription.push(this.fileService.getImage().subscribe((cardTable) => {
  ...
  public addToFavorite(card: Card): void {
    ...
    this.fileService.addToFavoriteImageCard(card.id, card.favorite);
    if (card.favorite) { this.playAudio('assets/image/addFavoriteImage.mp3'); } else { this.playAudio('assets/image/unsetFavoriteImage.mp3'); }
  }
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result != undefined) {
        if (result.invalid) {
              ...
              imageUpload.snapshotChanges().pipe(
                finalize(() => {
                  this.subscription.push(fileRef.getDownloadURL().subscribe((url) => {
                    ...
                    this.fileService.insertImageDetails(this.cardDetails);
          ...
          } else { this.fileService.insertImageDetails(this.cardDetails); }
        }
      }
    }
    );
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result != undefined) {
        if (result.invalid) {
            ...
            if (this.type.split('/')[0] === 'image') {
              ...
              imageUpload.snapshotChanges().pipe(
                finalize(() => {
                  this.subscription.push(fileRef.getDownloadURL().subscribe((url) => {
                    this.cardDetails.url = url;
                    this.fileService.updateFileDetails(this.cardDetails);
                    this.fileService.selectedImage = null;
                    this._snackBar.openFromComponent(SnackBarUploadedComponent, {
                      duration: 5000,
                      panelClass: ['pink-snackbar']
          ...
          } else { this.fileService.updateFileDetails(this.cardDetails); }
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result != undefined) {
        if (result.invalid) {
          ...
          if (result) {
            this.viewAllCard();
            this.fileService.deleteFileDetails(cardID);
            this.playAudio('assets/image/deleteButtonImage.mp3');
          }
        }
      }
    }
